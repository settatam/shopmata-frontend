                    ...
                    <div class="w-auto  lg:ml-7 lg:mr-2">
                        <div class="mb-6">
                            <div class="px-4 md:px-8 pb-8 pt-6  mb-6 bg-white">
                                <h1 class="text-2xl font-semibold">Edit Shipping Rate</h1>
                                <p class=" text-gray-500">Select a state and the cities within that state you can deliver to. Set a delivery rate and how long it will take to deliver items. <a class="text-indigo-700 underline cursor-pointer" @click="this.videoPop=true">Watch a demo</a></p>                       
                                    ...
                                    <div class="w-auto relative" >
                                        ...
                                        <input :class="{'border-red-600': v$.name.$error, 'border-gray-300': !v$.name.$error}" type="text"  class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="" v-model="rates.name" required/>
                                        <error-icon class="absolute top-8 right-0" v-show="bodyError && !rates.name.length "/>
                                    </div>

                                    <div class="mt-1">
                                        <p class="text-red-600 text-xs" v-if="v$.name.$error">
                                    ...
                                    <div class="w-auto relative">
                                        ...
                                        <input :class="{'border-red-600': v$.price.$error, 'border-gray-300': !v$.price.$error}" type="number"  class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder=""  v-model="rates.price" required/>
                                        <error-icon class="absolute top-8 right-0" v-show="bodyError && !rates.price.length "/>
                                    </div>

                                    <div class="mt-1">
                                        <p class="text-red-600 text-xs" v-if="v$.price.$error">
                                    ...
                                    <div class="w-full mt-4 ">
                                        ...
                                        <div class="mt-1">
                                            <p class="text-red-600 text-xs" v-if="v$.is_domestic.$error">
                                                {{ v$.is_domestic.$errors[0].$message }}
                                            </p>
                                        </div>
                                        ...
                                        <div class="mt-1">
                                            <p class="text-red-600 text-xs" v-if="v$.match_all_condition.$error">
                                                {{ v$.match_all_condition.$errors[0].$message }}
                                            </p>
                                        </div>
                                            ...
                                            <template v-for="(datum, index) in rateStore.conditions" :key="index" >
                                                <div class="flex flex-wrap justify-between lg:flex-row lg:justify-between mt-4">
                                                    ...
                                                    <TrashIcon v-if="conditions.length>1 && index !=0" class="w-5 h-5 text-red-500 my-auto cursor-pointer" @click="removeCondition(index)"/>
                                        ...
                                        <div class="flex mt-4">
                                            <div class="mr-2 w-full mb-3 relative">
                                                ...
                                                <textarea :class="{ 'border-red-600':v$.description.$error, 'border-gray-300': !v$.description.$error}"
                                    ...
                                    <button v-if="!v$.$error" :disabled="v$.$error" :class="{ disabled: loading,'opacity-25 cursor-not-allowed': loading }"
                                        class="text-white bg-indigo-700 rounded-md px-4 py-3 my-5 ml-5 w-1/6 flex justify-center" @click="submit">
                                        <svg
                                            v-if="loading"
                                            role="status"
                                    ...
                                    <button v-else class="text-white bg-indigo-700 rounded-md px-8 py-3 my-5 ml-5 w-1/6" @click="submit">
                                        Save
                                    </button>
                                </div>
...
export default {
    ...
    setup(props){
        ...
        const rates = reactive({ name:rateStore.name,
