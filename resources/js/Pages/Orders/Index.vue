                  ...
                  <table class="min-w-full divide-y divide-gray-200 table-fixed">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr v-for="order in orders.data" :key="order.id" class="bg-white">
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap text-sm text-gray-500"> {{ order.items.length == 1 ? '1 item' : order.items.length + 'Items'  }} </td>
                        <td class="px-6 py-4 text-right whitespace-nowrap text-sm text-gray-500">
...
export default {
  props: {
        ...
        computed: {
            ...
            numberOfItems() {
                return this.orders.items.length == 1 ? '1 Item' : this.orders.items.length + 'Items';
            }
        },
        methods: {
            next() {
