          ...
          <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            <div>
              <div class="mx-auto flex items-center justify-center">
                <h3 class="font-semibold text-2xl pb-5">Add a new {{this.text}}</h3> 
              </div>
              <div class="mt-3 text-center sm:mt-5">
                <div class="flex justify-between mb-8 py-1.5 bg-gray-100 -mx-6" v-if="this.text == 'layout' || this.text == 'template' || this.text == 'asset'">
                    <p class="lg:mr-4 text-base lg:text-lg  text-center my-auto lg:ml-11 ml-5  py-2 w-4.5/10" :class="activeUpload ? 'text-black bg-white':''" @click="toggleBtn" >Upload a file</p>
                    <p class="lg:mr-11 mr-5 text-base lg:text-lg  text-center my-auto  ml-4 py-2 w-4.5/10" :class=" activeBlank ? 'text-black bg-white':''" @click="toggleBtn">Create a blank file</p>
                </div>
                            
            <div v-if="activeUpload">
                <div class="flex ml-4" v-if="this.text == 'layout' || this.text == 'template' || this.text == 'asset'">
                    <label for="file-upload" class="custom-file-upload mr-5 mb-7" @click="filecount">Choose a file</label>
                    <input id="file-upload" type="file" multiple/><span class="mt-2" v-if="this.count== 0"> No file chosen</span> <span class="mt-2" v-else-if="this.count==1">{{this.count}} file chosen </span><span class="mt-2"  v-else> {{this.count}} files chosen </span>
                </div>
            </div>
            ...
            <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
              <button type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-cyan-700 text-base font-medium text-white focus:outline-none sm:ml-3 sm:w-auto sm:text-sm" @click="upload()">
                <i class="fas fa-spinner fa-pulse text-white m-2"></i> Submit
              </button>
