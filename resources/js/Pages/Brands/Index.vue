          ...
          <div class="px-4 sm:px-6 lg:mx-auto lg:px-8">
            <div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
              ...
              <div class="mt-6 flex space-x-3 md:mt-0 md:ml-4">
                <inertia-link href="brands/create" type="button" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500">
                  Create New Brand
                </inertia-link>
              </div>
            </div>
                ...
                <div class="align-middle min-w-full overflow-x-auto shadow overflow-hidden sm:rounded-lg">
                  <table class="min-w-full divide-y divide-gray-200 table-fixed">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr v-for="brand in brands.data" :key="brand.id" class="bg-white">
                        <td class="max-w-0 w-1/3 px-6 py-4 items-center whitespace-nowrap text-gray-900">
                          <div class="flex items-center">
                        ...
                        <td class="px-6 py-4 text-left whitespace-nowrap text-sm text-gray-500">
                          <time :datetime="brand.created_at">{{ brand.created_at }}</time>
                        </td>
                        <td class="hidden px-6 py-4 whitespace-nowrap text-sm text-gray-500 md:block">
                          <span :class="[statusStyles[brand.status], 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize']">
                            {{ brand.status }}
                          </span>
                        </td>
                        <td class="px-6 py-4 text-right whitespace-nowrap text-sm text-gray-500"> Edit </td>
                      </tr>
                    </tbody>
                  </table>
                  <!-- Pagination -->
