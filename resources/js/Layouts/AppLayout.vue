        ...
        <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
          <div class="relative flex-1 flex flex-col max-w-xs w-full pt-5 pb-4 bg-cyan-700">
            ...
            <div class="flex-shrink-0 flex items-center px-4">
              <img class="h-8 w-auto" src="https://fashionerize.nyc3.cdn.digitaloceanspaces.com/shopmata/shopmata-logo.png" alt="Shopmata Logo" />
            </div>
            <nav class="mt-5 flex-shrink-0 h-full overflow-y-auto" aria-label="Sidebar">
              ...
              <div>
                <div class="px-2 space-y-1">
                  <inertia-link v-for="item in secondaryNavigation" :key="item.name" :href="item.href" class="group flex items-center px-2 py-2 text-base font-medium rounded-md text-cyan-100 hover:text-white hover:bg-cyan-600">
          ...
          <nav class="mt-5 flex-1 flex flex-col overflow-y-auto" aria-label="Sidebar">
            <template v-for="item in navigation" :key="item.name">
              ...
              <Disclosure as="div" v-else class="space-y-1" v-slot="{ open }">
                <DisclosureButton :class="[item.current ? 'bg-cyan-800 text-white' : 'text-cyan-100 hover:text-white hover:bg-cyan-600', 'group w-full flex items-center pl-2 pr-1 py-2 text-left text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-500']">
                  <component :is="item.icon" class="mr-3 flex-shrink-0 h-6 w-6 text-cyan-200" aria-hidden="true" />
                  <span class="flex-1">
                    {{ item.name }}
                  </span>
                  <svg :class="[open ? 'text-cyan-400 rotate-90' : 'text-cyan-300', 'ml-3 flex-shrink-0 h-5 w-5 transform group-hover:text-cyan-400 transition-colors ease-in-out duration-150']" viewBox="0 0 20 20" aria-hidden="true">
            ...
            <div class="mt-6 pt-6">
              <div class="px-2 space-y-1">
                <inertia-link v-for="item in secondaryNavigation" :key="item.name" :href="item.href" class="group flex items-center px-2 py-2 text-sm leading-6 font-medium rounded-md text-cyan-100 hover:text-white hover:bg-cyan-600">
                  <component :is="item.icon" class="mr-4 h-6 w-6 text-cyan-200" aria-hidden="true" />
                  {{ item.name }}
                </inertia-link>
              </div>
            ...
            <Menu as="div" class="ml-3 relative">
              <div>
                <MenuButton class="max-w-xs bg-white rounded-full flex items-center text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 lg:p-2 lg:rounded-md lg:hover:bg-gray-50">
                  <p class="h-10 w-10 rounded-full capitalize bg-blue-400 text-white text-center text-sm py-2.5 font-semibold mr-4">{{$page.props.user.first_name.charAt(0)}}{{$page.props.user.last_name.charAt(0)}}</p>
                 <!--  <span class="hidden ml-3 text-gray-700 text-sm font-medium lg:block"><span class="sr-only">Open user menu for </span>{{$page.props.user.first_name}} {{$page.props.user.first_name}}</span> -->
                  <ChevronDownIcon class="hidden flex-shrink-0 ml-1 h-5 w-5 text-black font-extrabold lg:block" aria-hidden="true" />
                </MenuButton>
              </div>
